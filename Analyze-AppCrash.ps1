...
function Analyze-AppCrash
{
    ...
    foreach ($DumpFile in $DumpFiles)
    {
        ...
        {
            Write-Warnig "Crash Dump {0} cloud not be analyzed" -f $DumpFile.Name
        }
        ...
        if (!$FailureImageName)
        {
             Write-Warnig "Crash Dump {0} does not contain a FAILURE_IMAGE_NAME" -f $DumpFile.Name
        }

        if (!$ProcessName)
        {
             Write-Warnig "Crash Dump {0} does not contain a PROCESS_NAME" -f $DumpFile.Name
        }
